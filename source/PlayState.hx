...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		timeTxt.setFormat(Paths.font("ComicSansMSBold.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		scoreTxt.setFormat(Paths.font("ComicSansMSBold.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botplayTxt.setFormat(Paths.font("ComicSansMSBold.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	public function endSong():Void
	{
		...
		else
		{
			var achieve:String = checkForAchievement([
				'week1_nomiss', 'week2_nomiss', 'week3_nomiss', 'week4_nomiss', 'week5_nomiss', 'week6_nomiss', 'week7_nomiss', 'crumbcomplete', 'exocomplete',
				'ur_bad', 'ur_good', 'hype', 'two_keys', 'toastie', 'debugger', 'Crumb_FC', 'Weel_FC', 'Trip_FC', 'Cheese_FC', 'Exoweels_FC', 'Sane_FC',
				'Vestige_FC', 'Grape_FC', 'Mauve_FC', 'scrumby_bee', 'Ballin_FC', 'Yippeee_FC', 'Red_Dude_Beat', 'Absolute_Beat', 'Crumbled_Beat', 'smooch_fc'
			]);
	...
	private function checkForAchievement(achievesToCheck:Array<String> = null):String
	{
		...
		for (i in 0...achievesToCheck.length)
		{
			...
			if (!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled)
			{
				...
				{
					case 'week1_nomiss' | 'week2_nomiss' | 'week3_nomiss' | 'week4_nomiss' | 'week5_nomiss' | 'week6_nomiss' | 'week7_nomiss' | 'crumbcomplete' |
						'exocomplete':
						if (isStoryMode
							...
							&& !usedPractice)
						{
							...
							switch (weekName) // I know this is a lot of duplicated code, but it's easier readable and you can add weeks with different names than the achievement tag
							{
								...
								case 'weekcrumb':
									if (achievementName == 'crumbcomplete') unlock = true;
								case 'weekexo':
									if (achievementName == 'exocomplete') unlock = true;
							}
						}
					case 'ur_bad':
					...
					case 'Crumb_FC':
						if (Paths.formatToSongPath(SONG.song) == 'granule' && !usedPractice)
						{
							unlock = true;
						}
					case 'Weel_FC':
						if (Paths.formatToSongPath(SONG.song) == 'ambulant' && !usedPractice)
						{
							unlock = true;
						}
					case 'Trip_FC':
						if (Paths.formatToSongPath(SONG.song) == 'tripped' && !usedPractice)
						{
							unlock = true;
						}
					case 'Cheese_FC':
						if (Paths.formatToSongPath(SONG.song) == 'cheesefries' && !usedPractice)
						{
							unlock = true;
						}
					case 'Exoweels_FC':
						if (Paths.formatToSongPath(SONG.song) == 'caliber' && !usedPractice)
						{
							unlock = true;
						}
					case 'Sane_FC':
						if (Paths.formatToSongPath(SONG.song) == 'sane' && !usedPractice)
						{
							unlock = true;
						}
					case 'Vestige_FC':
						if (Paths.formatToSongPath(SONG.song) == 'vestige' && !usedPractice)
						{
							unlock = true;
						}
					case 'Grape_FC':
						if (Paths.formatToSongPath(SONG.song) == 'juiced' && !usedPractice)
						{
							unlock = true;
						}
					case 'Mauve_FC':
						if (Paths.formatToSongPath(SONG.song) == 'mauve' && !usedPractice)
						{
							unlock = true;
						}
					case 'scrumby_bee':
						if (Paths.formatToSongPath(SONG.song) == 'scrumbly-bee' && !usedPractice)
						{
							unlock = true;
						}
					case 'Ballin_FC':
						if (Paths.formatToSongPath(SONG.song) == 'ballin' && !usedPractice)
						{
							unlock = true;
						}
					case 'Yippeee_FC':
						if (Paths.formatToSongPath(SONG.song) == 'yippee' && !usedPractice)
						{
							unlock = true;
						}
					case 'Absolute_Beat':
						if (Paths.formatToSongPath(SONG.song) == 'godlike' && !usedPractice)
						{
							unlock = true;
						}
					case 'Red_Dude_Beat':
						if (Paths.formatToSongPath(SONG.song) == 'wrathful-vengeance' && !usedPractice)
						{
							unlock = true;
						}
					case 'Crumbled_Beat':
						if (Paths.formatToSongPath(SONG.song) == 'crumbled' && !usedPractice)
						{
							unlock = true;
						}
					case 'smooch_fc':
						if (Paths.formatToSongPath(SONG.song) == 'weel-x-crumb-sipping-smal' && !usedPractice)
						{
							unlock = true;
						}
				}
